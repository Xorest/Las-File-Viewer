...
HeaderFile::HeaderFile(const QByteArray &bytes)
{
    if (bytes.size() == 227)
    {
        ...
        _projectId_GUID_data_1 = Converter::byteToLond(bytes.mid(8, 4));
        ...
        _offsetToPointData = Converter::byteToLond(bytes.mid(96, 4));
        _numberOfVariableLenghtRecords = Converter::byteToLond(bytes.mid(100, 4));
        _pointDataFormatId = Converter::byteToUchar(bytes.mid(104, 1));
        _pointDataRecordLength = Converter::byteToInt(bytes.mid(105, 2));
        _numberOfPointRecords  = Converter::byteToLond(bytes.mid(107, 4));

        int pos = 111;
        for (int i = 0; i < 5; i++)
        {
            _numberOfPointsByReturn.append(Converter::byteToLond(bytes.mid(pos, 4)));
            pos = pos + 4;
        }
